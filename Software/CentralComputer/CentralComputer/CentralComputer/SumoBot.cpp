...
SumoBot::SumoBot(int lifes)
{
	...
	//Create TCP-Server 
	_TCPS = new TCPServer(1);
	_TCPS->openServer(); //Sleeping till 1 connected client 
...
bool SumoBot::setDirectionAndSpeed(int dir, int speed)
{
	...
	if (_TCPS->send(msg->c_str()) < 0)
	{
		cout << "SUMOBOT: Send failed!" << endl; 
		return -1; 
	}
	if (_TCPS->receive(&attackStatusBuffer, 1) < 0)
	{
