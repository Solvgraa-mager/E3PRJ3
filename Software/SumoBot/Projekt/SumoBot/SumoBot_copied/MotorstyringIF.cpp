...
void MotorstyringIF::setSpeedDir(char *speedDir)
{
		...
		if (speedDir[0] == '-') { Derection = 1; Input1 = 1; Input3 = 1; Input2 = 0; Input4 = 0; }     // Checker om der er negativ fortegn og sætter "Derection" derefter
		...
		{															// Og tiøfæjer vlrdien til "PWM_Size"
			...
			if (speedDir[i] == '|')										// Breaker hvis 
			{
				i = 5; PWM_Size--;
			}
		}

		if (speedDir[PWM_Size + 1 + Derection] == '-') { DerectionAngle = 1; }         // Checker om der er negativ fortegn og sætter "Derection" derefter
		else { DerectionAngle = 0; }

		Angle_Size = sizeof(speedDir) - PWM_Size - Derection - DerectionAngle - 2;            // Beregner størelsen af Angle_Size

		for (int i = sizeof(speedDir) - Angle_Size - 1; i < sizeof(speedDir) - 1; i++)   // Omregner Char til int og lægger værduen ind i ValueAngle
		{
